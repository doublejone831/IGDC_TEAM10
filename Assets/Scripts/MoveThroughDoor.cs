...
public class MoveThroughDoor : MonoBehaviour
{
    ...
    void movePoint() //문을 통해 이동
    {
        if (DoorTrigger == true)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (target == FirstDoor)
                {
                    player.transform.position = new Vector3(SecondDoor.transform.position.x, SecondDoor.transform.position.y, -1);

                    DoorTrigger = false;
                }
                else if (target == SecondDoor)
                {
                    player.transform.position = new Vector3(FirstDoor.transform.position.x, FirstDoor.transform.position.y, -1);
    ...
    private void OnTriggerEnter2D(Collider2D collision) //트리거 충돌 인식 라이브러리 함수
    {
